...
var validatePhone = function() {
    ...
    //var pattern = /^(1-)?\d{3}-\d{3}-\d{4}$/;                   // optional prefix of '1-'
    //var pattern = /^(1[-\.])?\d{3}[-\.]\d{3}[-\.]\d{4}$/;       // can use dashes or periods
    var pattern = /^(1[-\.\s])?(\()?\d{3}(\))?[-\.\s]\d{3}[-\.]\d{4}$/;   // can use parens around area code
